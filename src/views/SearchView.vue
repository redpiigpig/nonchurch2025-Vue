...
const highlightSnippet = (content, searchTerm) => {
  ...
  // 1. 先移除所有 HTML 標籤 (<div...>, <img...>, </a> 等)
  // 將標籤替換為 " " (空白)，避免文字黏在一起 (例如 </div><div> 變成連字)
  let plainText = content.replace(/<[^>]+>/g, " ");

  // 2. 移除 Markdown 圖片語法 ![alt](url) (如果有的話)
  plainText = plainText.replace(/!\[.*?\]\(.*?\)/g, " ");

  // 3. 整理空白：把連續的換行或空白縮減成一個空格
  plainText = plainText.replace(/\s+/g, " ").trim();

  // -----------------------------------------------------
  // 以下邏輯保持不變：在純文字中找關鍵字並截取
  // -----------------------------------------------------
  ...
  // 如果找不到 (可能關鍵字只存在於被我們刪掉的標籤屬性中)，就回傳開頭
  if (index === -1) {
  ...
  <div class="search-page-container">
    <div class="search-header-section">
      <div class="search-links">
        <span class="link-label">熱門關鍵字：</span>
        <a
          href="#"
          @click.prevent="
            inputQuery = '林貝克';
